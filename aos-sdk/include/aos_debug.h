 ...
 * \file aos_debug.h
 *
 * \brief Low level debug output, only functional when compiled with DEBUG
 *
...
#else
// Release mode.
#define aos_debug_printf(s...) /* no-op */
...
/*
 * \fn aos_assert_failed(condstr, file, line)
 *
...
/*
 * \fn aos_assert_warn(cond)
 *
 * \brief log a warning message if an assertion fails
 *
 * \input cond	condition that must be false for the assertion to fail.
 *
 */
