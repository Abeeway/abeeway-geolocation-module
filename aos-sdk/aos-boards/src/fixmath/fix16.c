...
fix16_t fix16_rad_to_degree(fix16_t rad)
{
	fix16_t tmp = fix16_div(fix16_from_int(180), fix16_pi);
	return fix16_mul(rad, tmp);
}
...
/*
 * Function fix16_shift
 * Action: Perform a right or left shift of the value
 * input:
 *   x: value
 *  shift: shift. negative value shift left, positive value shift right
 * output: none
 * return: the result of the operation
 * Comment:
 * This function divides the number by 2^shift
 */
fix16_t fix16_shift(fix16_t x, int8_t shift)
{
	bool neg = false;
