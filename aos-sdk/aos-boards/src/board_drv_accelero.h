...
typedef enum {
	...
	board_accelero_ioctl_type_get_current,	//!< Get the current (for consumption calculation)
...
typedef struct {
	...
	union {
		...
		uint32_t current;					//!< Current in nano-ampere. Follow board_accelero_ioctl_type_get_current
