...
#define GNSS_EHPE_MEASURE0_THRESHOLD 10000	//!<  Measurement 0: no threshold
#define GNSS_EHPE_MEASURE1_THRESHOLD 5000	//!<  Measurement 1: ehphe < 50m
#define GNSS_EHPE_MEASURE2_THRESHOLD 2500	//!<  Measurement 2: ehphe < 25m
#define GNSS_EHPE_MEASURE3_THRESHOLD 2000	//!<  Measurement 3: ehphe < 20m
#define GNSS_EHPE_MEASURE4_THRESHOLD 1500	//!<  Measurement 4: ehphe < 15m
#define GNSS_EHPE_MEASURE5_THRESHOLD 1000	//!<  Measurement 5: ehphe < 10m
#define GNSS_EHPE_MEASURE6_THRESHOLD 500	//!<  Measurement 6: ehphe < 5m
#define GNSS_EHPE_MEASURE7_THRESHOLD 300	//!<  Measurement 7: ehphe < 3m
...
static void _gnss_event_cb(aos_gnss_event_info_t *info, void * user_arg)
{
	...
	case aos_gnss_event_pseudo_range:
		...
		if (lctx.agps_monitor) {
			...
			lora_gnss_send(info->prn_report); //&lctx.gnss_prn_report);
		}
...
static const aos_gnss_configuration_t _gnss_config = {
		.constellations = aos_gnss_cfg_constellation_gps_only,
		.cbfn = _gnss_event_cb,
		.cbarg = NULL,
		.fix_period = AOS_GNSS_MIN_GPS_FIX_PERIOD,
		.track_period = AOS_GNSS_MIN_TRACKING_PERIOD,
		.filter_mask = AOS_GNSS_MSG_MSK_ALLOW_PSEUDO_RANGE,
...
/*static cli_parser_status_t _cmd_geoloc_gnss_start(void* arg, int argc, char *argv[]){
	//Function to start the Gnss scanning and reception of power range data.
	// Check arg


	_cmd_gnss_open(arg, argc, argv);
	lora_gnss_send(&lctx.gnss_prn_report);
...
static const cli_parser_cmd_t _cmd_geoloc_tab_alm[] = {
...
static const cli_parser_cmd_t _cmd_geoloc_tab_gnss_constell[] = {
...
static const cli_parser_cmd_t _cmd_geoloc_tab_gnss_counters[] = {
...
static const cli_parser_cmd_t _cmd_geoloc_tab_tab_gnss_show[] = {
...
static const cli_parser_cmd_t _cmd_geoloc_tab_gnss_prn_filter_tab[] = {
...
static const cli_parser_cmd_t _geoloc_gnss_table[] = {
		//PARSER_CMD_FUNC("-start", "Open GNSS service and activate handlers", _cmd_geoloc_gnss_start, CLI_ACCESS_ALL_LEVELS),
		...
		PARSER_CMD_TAB("-show", "Show fix, tracking or PRN", _cmd_geoloc_tab_tab_gnss_show, CLI_ACCESS_ALL_LEVELS),
		...
		PARSER_CMD_TAB("-almanac","GNSS almanac commands", _cmd_geoloc_tab_alm, CLI_ACCESS_ALL_LEVELS ),
		PARSER_CMD_TAB("-constel", "Get/set constellation", _cmd_geoloc_tab_gnss_constell, CLI_ACCESS_ALL_LEVELS),
		PARSER_CMD_FUNC("-restart", "Restart a GNSS acquisition", _cmd_gnss_restart, CLI_ACCESS_ALL_LEVELS),
		PARSER_CMD_TAB("-prn-filter", "Get/set PRN filter", _cmd_geoloc_tab_gnss_prn_filter_tab, CLI_ACCESS_ALL_LEVELS),
		PARSER_CMD_TAB("-counters", "Get/clear counters", _cmd_geoloc_tab_gnss_counters, CLI_ACCESS_ALL_LEVELS),
		PARSER_CMD_END
};
...
CLI_COMMAND_TAB_REGISTER(geolocGnss,"Padawan commands GNSS (MT3333)", _geoloc_gnss_table, CLI_ACCESS_ALL_LEVELS );
